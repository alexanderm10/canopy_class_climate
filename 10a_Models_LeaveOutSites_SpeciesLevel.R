# The unbalanced representation of samples and climate across sites continues to be
# a sticking point for reviews.  Rather than revert to showing single-site analyses 
# that are unable to leverage the power of multiple sites, we will follow their suggestion of a "leave-one-out" approach
...
for(SPP in spp.use){
  ...
  for(SITE in unique(dat.spp$Site.Code)){
    ...
    } # End Species-Level Fit
  

  
    mod.comp[mod.comp$Species==SPP & mod.comp$Site.Removed==SITE & mod.comp$Model=="climate.spp", "r.sq"] <- summary(gam.clim.spp$gam)$r.sq # R-squared
    ...
    mod.comp[mod.comp$Species==SPP & mod.comp$Site.Removed==SITE & mod.comp$Model=="climate.cc", "r.sq"] <- summary(gam.clim.cc$gam)$r.sq # R-squared
