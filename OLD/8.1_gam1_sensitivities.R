...
n <- 100
model.pred <- post.distns(model.gam=gam1, model.name="spp.cc_response", newdata=test, vars=predictors.all, n=n, terms=F)
...
mean.model <- aggregate(model.pred2$mean, by = model.pred2[, c("spp.cc", "Site.Code", "Year")], FUN=mean, na.rm=T)
names(mean.model)[names(mean.model)=="x"] <- c("BAI.mean") 
mean.model[,"BAI.lwr"] <- aggregate(model.pred2$mean, by=model.pred2[,c("spp.cc", "Site.Code", "Year")], FUN=quantile, probs=0.025, na.rm=T)[,"x"]
mean.model[,"BAI.upr"] <- aggregate(model.pred2$mean, by=model.pred2[,c("spp.cc", "Site.Code", "Year")], FUN=quantile, probs=0.975, na.rm=T)[,"x"]
head(mean.model)
...
mean.rw <- aggregate(test$BA.inc, by=test[, c("spp.cc", "Site.Code", "Year")], FUN=mean, na.rm=T)
...
mean.rw[,"BAI.lwr"] <- aggregate(test$BA.inc, by=test[,c("spp.cc", "Site.Code", "Year")], FUN=quantile, probs=0.025, na.rm=T)[,"x"]
mean.rw[,"BAI.upr"] <- aggregate(test$BA.inc, by=test[,c("spp.cc", "Site.Code", "Year")], FUN=quantile, probs=0.975, na.rm=T)[,"x"]
...
pdf("figures/prelim_figures/gam1/gam1_sanitycheck1.pdf", width= 13, height = 8.5)
for(i in unique(mean.rw$Site.Code)){
  print(ggplot(data=mean.rw[mean.rw$Site.Code==i,]) + facet_wrap(spp.cc ~ Site.Code, scales="fixed") + theme_bw() +
  	# plot the data
...
pdf("figures/prelim_figures/gam1/gam1_sanitycheck2.pdf", width= 13, height = 8.5)
...
gam1.weights <- factor.weights(model.gam = gam1, model.name = "spp.cc_response", newdata = test, extent = "", vars = vars, limiting=T)

summary(gam1.weights)
summary(test)
gam1.weights[,c("BA.inc", "spp.cc")] <- test[,c("BA.inc", "spp.cc")] # Adding in factors we forgot
...
save(gam1.weights, file="processed_data/gam_weights/gam1_weights.Rdata")
