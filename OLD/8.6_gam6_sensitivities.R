...
n <- 100
model.pred <- post.distns(model.gam=gam6, model.name="canopy_spp_add", newdata=test, vars=predictors.all, n=n, terms=F)
...
mean.model <- aggregate(model.pred2$mean, by = model.pred2[, c("Species", "Canopy.Class", "Site.Code", "Year")], FUN=mean, na.rm=T)
names(mean.model)[names(mean.model)=="x"] <- c("BAI.mean") 
mean.model[,"BAI.lwr"] <- aggregate(model.pred2$mean, by=model.pred2[,c("Species", "Canopy.Class", "Site.Code", "Year")], FUN=quantile, probs=0.025, na.rm=T)[,"x"]
mean.model[,"BAI.upr"] <- aggregate(model.pred2$mean, by=model.pred2[,c("Species", "Canopy.Class", "Site.Code", "Year")], FUN=quantile, probs=0.975, na.rm=T)[,"x"]
head(mean.model)
...
mean.rw <- aggregate(test$BA.inc, by=test[, c("Species","Canopy.Class", "Site.Code", "Year")], FUN=mean, na.rm=T)
...
mean.rw[,"BAI.lwr"] <- aggregate(test$BA.inc, by=test[,c("Species","Canopy.Class", "Site.Code", "Year")], FUN=quantile, probs=0.025, na.rm=T)[,"x"]
mean.rw[,"BAI.upr"] <- aggregate(test$BA.inc, by=test[,c("Species","Canopy.Class", "Site.Code", "Year")], FUN=quantile, probs=0.975, na.rm=T)[,"x"]
...
sanity.gam6.df <- merge(mean.model2, mean.rw, all.x=T, all.y=T)
summary(sanity.gam6.df)

sanity.gam6.df$log.BAI <- log(sanity.gam6.df$BAI.mean)
summary(sanity.gam6.df[is.na(sanity.gam6.df$mod.mean),])
head(sanity.gam6.df[is.na(sanity.gam6.df$mod.mean),])

gam6.resid <- resid(gam6)
hist(gam6.resid)
test$resid.gam6 <- resid(gam6)
plot(log(BA.inc) ~ resid.gam6, data=test)
...
pdf("figures/prelim_figures/gam6/gam6_sanitycheck1.pdf", width= 13, height = 8.5)
for(i in unique(mean.rw$Site.Code)){
print(ggplot(data=mean.rw[mean.rw$Site.Code==i,]) + facet_wrap(Canopy.Class ~ Species, scales="fixed") + theme_bw() +
	# plot the data
	geom_ribbon(aes(x=Year, ymin=BAI.lwr, ymax=BAI.upr), alpha=0.5) +
	geom_line(aes(x=Year, y=BAI.mean), size=1) +
	# Plot our model
	geom_ribbon(data=mean.model[mean.model$Site.Code==i,], aes(x=Year, ymin=exp(BAI.lwr), ymax=exp(BAI.upr)), fill="red3", alpha=0.3) +
	geom_line(data=mean.model[mean.model$Site.Code==i,], aes(x=Year, y=exp(BAI.mean)), color="red3", alpha=0.8, size=1) +
	labs(title=paste("Gamm Model vs. Data",i,sep="."), x="Year", y="BAI"))
}
dev.off()
...
pdf("figures/prelim_figures/gam6/gam6_sanitycheck2.pdf", width= 13, height = 8.5)
ggplot(data=test[test$TreeID %in% sanity2.trees,]) + facet_wrap(TreeID~ spp.cc, scales="fixed") + theme_bw() +
	# plot the data
...
gam6.weights <- factor.weights(model.gam = gam6, model.name = "canopy_species_add_response", newdata = test, extent = "", vars = vars, limiting=T)

summary(gam6.weights)
summary(test)
gam6.weights[,c("BA.inc", "Canopy.Class", "Species")] <- test[,c("BA.inc", "Canopy.Class","Species")] # Adding in factors we forgot

gam6.weights[,c("fit.full.bai", "tmean.bai", "precip.bai", "dbh.bai")] <- exp(gam6.weights[,c("fit.full", "fit.tmean", "fit.precip", "fit.dbh.recon")])
summary(gam6.weights)

# Just the weights of tmean and Precip, ignoring size
vars2 <- c("tmean.bai", "precip.bai", "dbh.bai")
fit.spline2 <- rowSums(abs(gam6.weights[,vars2]), na.rm=T)
for(v in vars2){
	gam6.weights[,paste("weight", v, "2", sep=".")] <- gam6.weights[,v]/fit.spline2
}
summary(gam6.weights)

cols.weights <- c("weight.tmean.bai.2", "weight.precip.bai.2", "weight.dbh.bai.2")
for(i in 1:nrow(gam6.weights)){
	fweight <- abs(gam6.weights[i,cols.weights])
	gam6.weights[i,"max2"] <- max(fweight, na.rm=T)
	gam6.weights[i,"factor.max2"] <- c("tmean", "precip", "dbh.recon")[which(fweight==max(fweight))]
}
gam6.weights$factor.max2 <- as.factor(gam6.weights$factor.max2)
summary(gam6.weights)

save(gam6.weights, file="processed_data/gam_weights/gam6_weights.Rdata")
