...
gam5 <- gam(log(BA.inc)~ s(tmean, k=3, by=Species) +
              s(precip, k=3, by=Species) +
            ...
            # random=list(Site=~1, PlotID=~1, TreeID=~1),
            data=test)
summary(gam5)$r.sq # R-squared
summary(gam5)$dev.expl # explained deviance
anova(gam5) 
AIC(gam5)
...
# ------
# # make a matrix with spp & canopy class
df.spp.cc <- matrix(unlist(stringr::str_split(new.dat2$spp.cc, "[.]")), ncol=2, byrow=T)
head(df.spp.cc)
summary(df.spp.cc)

summary(new.dat2)

new.dat2$Species <- as.factor(df.spp.cc[,1])
new.dat2$Canopy.Class <- as.factor(df.spp.cc[,2])
new.dat2$Canopy.Class <- factor(new.dat2$Canopy.Class, levels=c("Canopy", "I", "U"))
...
ggplot(data=ci.terms.graph[ci.terms.graph$Effect %in% c("precip"), ]) + 
  facet_wrap(~Species) +
  geom_line(aes(x=x, y=1), linetype="dashed")+
  geom_ribbon(aes(x=x, ymin=exp(lwr), ymax=exp(upr), fill=Canopy.Class), alpha=0.5) +
  geom_line(aes(x=x, y=exp(mean), color=Canopy.Class)) +
  coord_cartesian(ylim=c(0.5, 1.5), xlim=range(test$precip)) +
  scale_fill_manual(values=c("#0072B2", "#009E73", "#E69F00"))+
  scale_color_manual(values=c("#0072B2", "#009E73", "#E69F00"))+
  theme_bw()+
  labs(x = "Climate Variable", y = expression(bold(paste("Effect on BAI (mm"^"2", 							"y"^"-1",")"))))+
...
ggplot(data=ci.terms.graph[ci.terms.graph$Effect %in% c("dbh.recon"), ]) + 
  ...
  geom_ribbon(aes(x=x, ymin=exp(lwr), ymax=exp(upr), fill=Canopy.Class), alpha=0.5) +
  geom_line(aes(x=x, y=exp(mean), color=Canopy.Class)) +
  coord_cartesian(ylim=c(0, 10), xlim=range(test$dbh.recon)) +
  scale_fill_manual(values=c("#0072B2", "#009E73", "#E69F00"))+
  scale_color_manual(values=c("#0072B2", "#009E73", "#E69F00"))+
  theme_bw()+
  labs(x = "Climate Variable", y = expression(bold(paste("Effect on BAI (mm"^"2", 							"y"^"-1",")"))))+
...
ggplot(data=g1.ci.out[g1.ci.out$Effect %in% c("tmean", "precip", "dbh.recon"),]) + 
...
pdf("figures/prelim_figures/gam1_sensitivities_truncated_combo_nosize.pdf", width= 13, height = 8.5)		
ggplot(data=g1.ci.out[g1.ci.out$Effect %in% c("tmean", "precip"),]) + 
  facet_grid(Species~Effect, scales = "free") +
...
# SOurce & run the function
source("0_Calculate_GAMM_Posteriors.R")
g2.ci.terms.pred2 <- post.distns(model.gam=gam2, model.name="canopy_only", n=n, newdata=new.dat2, vars=predictors.all, terms=T)

g2.ci.out2 <- g2.ci.terms.pred2$ci # separting out the confidence interval 
g2.ci.out2[,predictors.all[!predictors.all %in% vars.num]] <- new.dat2[,predictors.all[!predictors.all %in% vars.num]] # copying over our factor labels
...
ggplot(data=g2.ci.out2[g2.ci.out2$Effect %in% c("tmean", "precip"), ]) + 
  ...
  scale_color_manual(values=c("#0072B2", "#009E73", "#E69F00")) +
  scale_fill_manual(values=c("#0072B2", "#009E73",  "#E69F00")) +
  coord_cartesian(ylim=c(0.5, 1.35)) +
  theme_bw()+
  labs(x = "Climate Variable", y = expression(bold(paste("Effect on BAI (mm"^"2","y"^"-1",")"))))+
  theme(axis.line.x = element_line(color="black", size = 0.5),
        axis.line.y = element_line(color="black", size = 0.5))+
...
for(v in vars.fac){
  ...
  # Get rid of unimportant levels for everything else
  l1 <- unique(new.dat4[,v])[1]
  new.dat4 <- new.dat4[new.dat4[,v]==l1,]
}
summary(new.dat4)


source("0_Calculate_GAMM_Posteriors.R")
g4.ci.terms.pred <- post.distns(model.gam=gam4, model.name="Site_level", n=n, newdata=new.dat4, vars=predictors.all, terms=T)

g4.ci.out <- g4.ci.terms.pred$ci # separting out the confidence interval 
g4.ci.out[,predictors.all[!predictors.all %in% vars.num]] <- new.dat4[,predictors.all[!predictors.all %in% vars.num]] # copying over our factor labels
...
# DBH
for(s in unique(test$Species)){
  dbh.min <- min(test[test$Species==s, "dbh.recon"])
  dbh.max <- max(test[test$Species==s, "dbh.recon"])
  
  ci.terms.graph$x <- ifelse(ci.terms.graph$Species!=s | ci.terms.graph$Effect!="dbh.recon" | (ci.terms.graph$x>=dbh.min & ci.terms.graph$x<=dbh.max), ci.terms.graph$x, NA)
  
}

# Temp
for(s in unique(test$Species)){
  temp.min <- min(test[test$Species==s, "tmean"])
  temp.max <- max(test[test$Species==s, "tmean"])
  
  ci.terms.graph$x <- ifelse(ci.terms.graph$Species!=s | ci.terms.graph$Effect!="tmean" | (ci.terms.graph$x>=temp.min & ci.terms.graph$x<=temp.max), ci.terms.graph$x, NA)
  
}

# Precip	
for(s in unique(test$Species)){
  precip.min <- min(test[test$Species==s, "precip"])
  precip.max <- max(test[test$Species==s, "precip"])
  
  ci.terms.graph$x <- ifelse(ci.terms.graph$Species!=s | ci.terms.graph$Effect!="precip" | (ci.terms.graph$x>=precip.min & ci.terms.graph$x<=precip.max), ci.terms.graph$x, NA)
...
load("processed_data/gam_results/gam5_species_canopy_add.Rdata")		
...
for(v in vars.fac){
  if(v %in% var.smooth) next # keep all levels for our "by" variable
  # Get rid of unimportant levels for everything else
  l1 <- unique(new.dat5[,v])[1]
  new.dat5 <- new.dat5[new.dat5[,v]==l1,]
}
new.dat5$spp.cc <- as.factor(paste(new.dat5$Species, new.dat5$Canopy.Class, sep="."))
summary(new.dat5)

source("0_Calculate_GAMM_Posteriors.R")
g5.ci.terms.pred <- post.distns(model.gam=gam5, model.name="Additive", n=n, newdata=new.dat5, vars=predictors.all, terms=T)

g5.ci.out <- g5.ci.terms.pred$ci # separting out the confidence interval 
g5.ci.out[,predictors.all[!predictors.all %in% vars.num]] <- new.dat5[,predictors.all[!predictors.all %in% vars.num]] # copying over our factor labels
g5.ci.out$x <- as.numeric(g5.ci.out$x) # making x numeric; will make factors NA; NA's are ok here
summary(g5.ci.out)

# Convert mean, lwr, upr to BAI units
g5.ci.out[,c("mean.bai", "lwr.bai", "upr.bai")] <- exp(g5.ci.out[,c("mean", "lwr", "upr")])
summary(g5.ci.out)

ci.terms.graph <- g5.ci.out
...
summary(ci.terms.graph)
save(ci.terms.graph, file="processed_data/gam5_response_graph.Rdata")

pdf("figures/prelim_figures/gam5_sensitivities_observed_tmean.pdf", width= 13, height = 8.5)
ggplot(data=ci.terms.graph[ci.terms.graph$Effect %in% "tmean", ]) + 
  facet_wrap(~Species) +
  geom_ribbon(aes(x=x, ymin=exp(lwr), ymax=exp(upr), fill=Canopy.Class), alpha=0.5) +
  geom_line(aes(x=x, y=exp(mean), color=Canopy.Class))+
...
pdf("figures/prelim_figures/gam5_sensitivities_observed_precip.pdf", width= 13, height = 8.5)
ggplot(data=ci.terms.graph[ci.terms.graph$Effect %in% "precip", ]) + 
  facet_wrap(~Species) +
  geom_ribbon(aes(x=x, ymin=exp(lwr), ymax=exp(upr), fill=Canopy.Class), alpha=0.5) +
  geom_line(aes(x=x, y=exp(mean), color=Canopy.Class))+
...
pdf("figures/prelim_figures/gam5_sensitivities_dbh_recon_observed.pdf", width= 13, height = 8.5)
ggplot(data=ci.terms.graph[ci.terms.graph$Effect %in% "dbh.recon", ]) + 
  # facet_wrap(~Species) +
  geom_ribbon(aes(x=x, ymin=exp(lwr), ymax=exp(upr), fill=Species), alpha=0.5) +
  geom_line(aes(x=x, y=exp(mean), color=Species))+
...
pdf("figures/prelim_figures/gam5_sensitivities_observed_combo.pdf", width= 13, height = 8.5)
ggplot(data=ci.terms.graph[ci.terms.graph$Effect %in% c("tmean", "precip","dbh.recon"), ]) + 
  facet_grid(Species~Effect, scales="free_x") +
  geom_ribbon(aes(x=x, ymin=exp(lwr), ymax=exp(upr), fill=Canopy.Class), alpha=0.5) +
  geom_line(aes(x=x, y=exp(mean), color=Canopy.Class))+
...
pdf("figures/prelim_figures/gam5_sensitivities_observed_combo_no_size.pdf", width= 13, height = 8.5)
ggplot(data=ci.terms.graph[ci.terms.graph$Effect %in% c("tmean", "precip"), ]) + 
  facet_grid(Species~Effect, scales="free_x") +
  geom_ribbon(aes(x=x, ymin=exp(lwr), ymax=exp(upr), fill=Canopy.Class), alpha=0.5) +
  geom_line(aes(x=x, y=exp(mean), color=Canopy.Class))+
...
summary(ci.terms.graph)
save(ci.terms.graph, file="processed_data/gam6_response_graph.Rdata")

pdf("figures/prelim_figures/gam6_sensitivities_observed_tmean.pdf", width= 13, height = 8.5)
ggplot(data=ci.terms.graph[ci.terms.graph$Effect %in% "tmean", ]) + 
  facet_wrap(~Species) +
  geom_ribbon(aes(x=x, ymin=exp(lwr), ymax=exp(upr), fill=Canopy.Class), alpha=0.5) +
  geom_line(aes(x=x, y=exp(mean), color=Canopy.Class))+
...
pdf("figures/prelim_figures/gam6_sensitivities_observed_precip.pdf", width= 13, height = 8.5)
ggplot(data=ci.terms.graph[ci.terms.graph$Effect %in% "precip", ]) + 
  facet_wrap(~Species) +
  geom_ribbon(aes(x=x, ymin=exp(lwr), ymax=exp(upr), fill=Canopy.Class), alpha=0.5) +
  geom_line(aes(x=x, y=exp(mean), color=Canopy.Class))+
...
pdf("figures/prelim_figures/gam6_sensitivities_dbh_recon_observed.pdf", width= 13, height = 8.5)
ggplot(data=ci.terms.graph[ci.terms.graph$Effect %in% "dbh.recon", ]) + 
  # facet_wrap(~Species) +
  geom_ribbon(aes(x=x, ymin=exp(lwr), ymax=exp(upr), fill=Species), alpha=0.5) +
  geom_line(aes(x=x, y=exp(mean), color=Species))+
...
pdf("figures/prelim_figures/gam6_sensitivities_observed_combo.pdf", width= 13, height = 8.5)
ggplot(data=ci.terms.graph[ci.terms.graph$Effect %in% c("tmean", "precip","dbh.recon"), ]) + 
  facet_grid(Species~Effect, scales="free_x") +
  geom_ribbon(aes(x=x, ymin=exp(lwr), ymax=exp(upr), fill=Canopy.Class), alpha=0.5) +
  geom_line(aes(x=x, y=exp(mean), color=Canopy.Class))+
...
pdf("figures/prelim_figures/gam6_sensitivities_observed_combo_no_size.pdf", width= 13, height = 8.5)
ggplot(data=ci.terms.graph[ci.terms.graph$Effect %in% c("tmean", "precip"), ]) + 
  facet_grid(Species~Effect, scales="free_x") +
  geom_ribbon(aes(x=x, ymin=exp(lwr), ymax=exp(upr), fill=Canopy.Class), alpha=0.5) +
  geom_line(aes(x=x, y=exp(mean), color=Canopy.Class))+
