library(ggplot2)
library(car)
load(file="processed_data/gam_weights/gam1_weights.Rdata")
...
othervars <- c("Year", "Site", "spp.cc", "Model")
...
plot.rgb <- function(SITE, SPP.CC, SIZE){	geom_point(data=data.graph[data.graph$State==SITE & data.graph$group==SPP.CC,],aes(x=Year, y=fit.full), size=SIZE,
  		        color=rgb(abs(data.graph[data.graph$Site==SITE & data.graph$spp.cc==SPP.CC,"weight.tmean2"     ]), # red
                        abs(data.graph[data.graph$Site==SITE & data.graph$spp.cc==SPP.CC,"weight.dbh.recon2"     ]), # green
                        abs(data.graph[data.graph$Site==SITE & data.graph$spp.cc==SPP.CC,"weight.precip2"   ]))) }   # blue
...
save(data.graph, file="processed_data/gam_weights/gam1_weights_graph.Rdata")
...
ggplot(data.graph) + facet_grid(spp.cc~Site, scale="free") +
...
for(i in unique(data.graph$Site))
print(ggplot(data.graph[data.graph$Site==i,]) + facet_grid(spp.cc~Site, scale="free") +
...
for(i in unique(data.graph$Site)){
print(ggplot(data.graph[data.graph$Site==i,]) + facet_grid(spp.cc~Site) +
	# facet_wrap(~TreeID, scales="free_y", space="free") +
